...
void Hash::Inserir(const Funcionario& func) {
    unsigned int h = func.nascimento.Hash(mNroPosicoes);
    Noh* atual = new Noh(func);
    mElementos[h].Adicionar(atual);
}
...
float Hash::PorcentagemDeUso() const {
    ...
    float valor = float(cont)/mNroPosicoes;
    return valor;
}
...
unsigned Hash::QtdeAniversariantes(const Data& d) {
    ...
    for(int i=0;i<mNroPosicoes;i++){
        if(mElementos[i].mPrimeiro != NULL){
            Noh* atual = mElementos[i].mPrimeiro;
            while(atual != NULL){
                if(atual->mValor.FazAniversarioEm(d)){
                    cont++;
                }
                atual = atual->mProximo;
            }
